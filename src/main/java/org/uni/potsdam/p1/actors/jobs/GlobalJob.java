...
//      .slotSharingGroup("o2")
    ...
    SingleOutputStreamOperator<Measurement> counter3 = simpleConnect(operator1.union(operator2), global)
      .process(new SourceCounter(OPERATORS[2])
        .setMetricsOutput("lambdaIn", toAnalyser3)
        .setMetricsOutput("sos", toCoordinator))
      .name("Counter3");

    SingleOutputStreamOperator<Measurement> counter4 = simpleConnect(operator1.getSideOutput(toOperator4).union(operator2.getSideOutput(toOperator4)), global)
      .process(new SourceCounter(OPERATORS[3])
